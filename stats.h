...
static inline void sched_info_depart(struct rq *rq, struct task_struct *t)
{
	...
	static int printed = 0; //출력횟수 제한을 위해서 변수 설정
	if (strncmp(t ->comm, "cpu", 3) != 0) //t->comm 이름이 "cpu"로 시작하는 프로세스만
		return;

		//t 구조체에서 필요한 변수 획득하기. 몇가지 특별한 함수들도 사용되었다.
		pid_t upid = task_tgid_vnr(t);
		int nice = task_nice(t);
		struct sched_entity *se = &t -> se;
		u64 vruntime = se -> vruntime;
		s64 vdeadline = se -> deadline;
		unsigned long long cpu_burst = delta;

		//로그 출력 후 출력횟수를 1 올려준다.
		printk(KERN_INFO"[EEVDF] pid=%d nice=%d vruntime=%llu vdeadline=%llu cpu_burst=%llu", upid, nice, (unsigned long long)vruntime, (unsigned long long)vdeadline, cpu_burst);
		printed++;
